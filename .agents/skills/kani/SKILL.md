...
**When adding new `#[kani::proof]` functions, you MUST add them to tier lists in `scripts/verification/verify-kani.sh`.**
...
3. Add to tier list in `scripts/verification/verify-kani.sh`:

   ```bash
   TIER1_PROOFS=( ... "proof_my_invariant" )
   ```

4. Validate: `./scripts/verification/check-kani-coverage.sh`
...
span ([doc-code-sync.md](../doc-code-sync/SKILL.md#intra-doc-links-to-cfgkani-only-items)).
...
- [ ] Add to tier list in `scripts/verification/verify-kani.sh` and run `./scripts/verification/check-kani-coverage.sh`
