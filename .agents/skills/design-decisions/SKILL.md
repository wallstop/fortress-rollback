...
description: "Design Decision Log Pattern guidance for Fortress Rollback. Use when Architectural decisions, design alternatives, superseding prior choices."
---
...
Store logs under this skill's `references/` directory as plain text files by domain,
for example:

- `references/network.txt`
- `references/sync-layer.txt`

Read [the design-history index](references/index.md) before adding or superseding
an entry. Load only the domain file relevant to the current decision.
