...
description: "Defensive Programming -- Zero-Panic Production Code guidance for Fortress Rollback. Use when Implementing error handling, ensuring zero-panic compliance, validating inputs."
---
...
1. **Never trust a length from the wire.** Bound any size derived from network input against a caller-configured limit or explicit default *before* allocating, and use `checked_add` while accumulating so a crafted packet cannot overflow `usize`. Return a structured error on the bomb path instead of allocating. When a wire length is decoded into a wider type (e.g. a `u64` varint), range-check it against the limit *while it is still `u64`*, before any `as usize` narrowing: on a 32-bit target (`usize == u32`) the cast truncates the high bits and could turn a huge declared length into a small one that slips past the check (see `rle::checked_run_len`). Example: protocol input decode derives its RLE decoded-length limit from `ProtocolConfig::pending_output_limit` and the reference input size; standalone `rle::decode` uses `rle::DEFAULT_MAX_DECODED_LEN`. Avoid generic serde/bincode container decoding for untrusted bytes when the schema has `Vec`, `String`, or similar length-prefixed fields; parse those lengths explicitly, validate them against the actual input/config, reserve fallibly, and then copy/decode the bounded payload.
   When bounded bincode decoding still needs a consumed-byte count, use `network::codec::decode_bounded_with_consumed`; do not point peer-controlled callers at generic `codec::decode`.
...
## Symmetric teardown and single-send sites

When the same logical operation runs from more than one path, the set of steps must not diverge between paths — the classic bug is one abort path that cleans up less than another (e.g. a hot-join serve aborted on timeout cleared the joiner endpoint's `pending_output`, but the abort-on-disconnect path forgot to). Funnel **every** abort/teardown path through one helper that performs the complete cleanup, so a new caller cannot under-clean by omission (`P2PSession::abort_hot_join_serve` is the canonical example; both the Phase-4 timeout and the reserved-slot disconnect call it). The success path is deliberately separate when it must keep state the abort path discards.

Mirror this for retransmits: a "send once now, then resend each poll" shape double-sends on the triggering poll if the just-opened item is already in the resend set. Keep a **single send site** (the per-poll resend loop) and let the open step only enqueue — then each item is transmitted exactly once per poll, and poll-count/timeout accounting (`>=`, not `>`, for an inclusive "max N polls" bound) stays exact. The same applies to ack resends on the receiving side: one ack-send site per poll, never a resend loop plus a duplicate-arrival handler that both fire.

Make these bounds (serve timeout, ack-resend budget) configurable builder DEFAULTS, validated at the boundary (reject a `0` timeout that would abort before any handshake completes). Test per-poll send counts deterministically with a counting socket that tallies sends around each `poll_remote_clients` and asserts the delta is `<= 1` (see `tests/sessions/hot_join.rs` `CountingSocket`).
