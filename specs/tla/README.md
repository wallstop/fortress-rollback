...
| `SyncHandshakeV1Trace.tla` | generated by `verify-sync-handshake-traces.py` | ✓ CI | Strict hand-authored NDJSON trace contract: matching/mismatch/timeout pass and an exact duplicate-reply decrement mutation fails; the bounded raw runtime recorder exists, while its producer/normalization driver and refinement gate remain pending |
...
The no-instrumentation trace-contract spike lives under
`specs/tla/traces/sync-handshake-v1/`. Three hand-authored NDJSON behaviors constrain the real base
actions and are accepted; a fourth file derives from the accepted duplicate-reply behavior and
changes only `syncRemaining.p1` from 1 to 0, which TLC must reject. The Python wrapper validates
the schema and mutation relation before generating temporary TLA data, then fails closed on parse,
tool, timeout, incomplete-consumption, property, or exit-code drift. See the trace directory README
for the candidate runtime refinement table and its explicit gaps.
...
| `SyncHandshakeV1Trace` | `src/network/protocol/handshake_trace.rs` and transition hooks in `src/network/protocol/mod.rs` provide bounded raw events. The fixtures remain hand-authored; the builder/SimNet producer, normalization driver, and runtime-to-TLC gate are still pending. See `traces/sync-handshake-v1/README.md`. |
