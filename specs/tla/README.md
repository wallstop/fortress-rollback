...
| `SyncHandshakeV1Trace.tla` | generated by `verify-sync-handshake-traces.py` | ✓ CI | Strict hand-authored and runtime-derived NDJSON trace contract: matching/mismatch/timeout pass and exact duplicate-reply decrement mutations fail; the bounded recorder, two-peer SimNet producer, ID normalizer, and TLC refinement gate run together |
...
bounded safety model permits arbitrary message loss, overlapping fresh request IDs from a namespace
independent of the required successful-roundtrip count, and reordered replies over two fixed-width
configuration fields. A sync timeout
emits once but deliberately leaves the endpoint `Syncing` with retries enabled.

### SyncHandshakeV1Trace.tla

The trace contract lives under `specs/tla/traces/sync-handshake-v1/`. Three hand-authored NDJSON
behaviors constrain the real base actions and are accepted; a fourth file derives from the accepted
duplicate-reply behavior and changes only `syncRemaining.p1` from 1 to 0, which TLC must reject.
The same gate builds three fresh traces from real two-peer `P2PSession` endpoints over SimNet,
normalizes raw random request IDs without collapsing equality, and derives the same impossible
decrement from the observed duplicate row. The Python wrapper validates both manifests and mutation
relations before generating temporary TLA data, then fails closed on producer, parse, tool, timeout,
incomplete-consumption, property, or exit-code drift. See the trace directory README for the narrow
runtime refinement mapping and its explicit abstraction gaps.
...
|-------------|---------------|-------------|
| `SyncHandshakeV1.cfg` | PEERS={p1,p2}, NUM_SYNC_PACKETS=3, REQUEST_ID_COUNT=4, MAX_NETWORK=2, arbitrary loss, all 2×2 configs (safety only) | 936,756 distinct states (4,310,632 generated), ~8–15s workers-auto |
| `SyncHandshakeV1Fair.cfg` | same bounds, matching configs, fair delivery (no loss; liveness) | 46,224 distinct states (129,028 generated), ~3s workers-auto |
...
| `SyncHandshakeV1Trace` | `src/network/protocol/handshake_trace.rs` and transition hooks in `src/network/protocol/mod.rs` provide bounded raw events; the hidden builder option and `tests/simulation/trace_validation.rs` merge endpoint streams in deterministic SimNet poll order, normalize IDs, emit matching/mismatch/timeout traces, and derive the negative mutation consumed by `verify-sync-handshake-traces.py`. See `traces/sync-handshake-v1/README.md`. |
