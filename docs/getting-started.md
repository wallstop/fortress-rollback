...
description: Install Fortress Rollback and run a deterministic rollback session before connecting peers.
---

<!-- SYNC: This source doc syncs to wiki/Getting-Started.md. -->
...
values in the deterministic simulation. The [determinism model](specs/determinism-model.md) explains
the full contract.
...
- Read [The Game Loop](user-guide.md#the-game-loop), especially polling, event draining, and frame
  pacing.
- Choose measured defaults from [Network Tuning](tuning.md); do not guess at prediction windows.
- Decide how the game handles disconnects and desync evidence.
- Complete the [Production Checklist](production-checklist.md) before shipping.

Continue with the [User Guide](user-guide.md) for session types, events, spectators, hot-join,
