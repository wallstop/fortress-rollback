...
#
# Usage: ./scripts/verification/check-kani-coverage.sh [options]
#   ./scripts/verification/check-kani-coverage.sh           # Validate coverage
#   ./scripts/verification/check-kani-coverage.sh --fix     # Show what needs to be added
#   ./scripts/verification/check-kani-coverage.sh --verbose # Show detailed comparison
#
# Exit codes:
...
print_usage() {
    ...
    echo "are included in the tier lists in scripts/verification/verify-kani.sh."
}
...
main() {
    ...
    if [[ "$MISSING_COUNT" -gt 0 ]]; then
        ...
        if [[ "$FIX_MODE" == "true" ]]; then
            echo -e "${BLUE}To fix: Add these proofs to the appropriate tier in scripts/verification/verify-kani.sh${NC}"
            echo ""
    ...
    if [[ "$EXTRA_COUNT" -gt 0 ]]; then
        ...
        echo ""
        if [[ "$FIX_MODE" == "true" ]]; then
            echo -e "${BLUE}To fix: Remove these stale entries from scripts/verification/verify-kani.sh${NC}"
            echo ""
        fi
    fi
    ...
    if [[ "$has_errors" == "true" ]]; then
        ...
        echo "scripts/verification/verify-kani.sh to ensure they are run during CI verification."
        echo ""
