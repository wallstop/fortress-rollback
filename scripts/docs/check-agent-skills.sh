#!/usr/bin/env bash
#
# check-agent-skills.sh - Validate .agents/skills/ markdown files for code quality issues
#
# Checks:
...
#   4. Line count enforcement (500 lines max for SKILL.md and references)
#   5. 2>/dev/null in bash/shell code blocks (banned pattern)
#
# Usage:
#   ./scripts/docs/check-agent-skills.sh              # Check .agents/skills/ directory
#   ./scripts/docs/check-agent-skills.sh --fix        # Print fix suggestions
#   ./scripts/docs/check-agent-skills.sh --dir /path  # Check a custom directory
#
# Exit codes:
#   0 - No issues found
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -h|--help)
                ...
                echo "  --dir     Directory to scan (default: .agents/skills/)"
    ...
    # Default: derive .agents/skills/ relative to the script location.
    if [[ -z "$SEARCH_DIR" ]]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
        SEARCH_DIR="$REPO_ROOT/.agents/skills"
    fi
}
...
main() {
    ...
    echo -e "${BOLD}Checking .agents/skills/ markdown files for code quality issues...${NC}"
    echo ""
