#!/usr/bin/env bash
#
# test_check_agent_skills.sh - Tests for check-agent-skills.sh
#
...
#
# Usage:
#   ./scripts/tests/test_check_agent_skills.sh
#
# Exit codes:
#   0 - All tests passed
...
CHECK_SCRIPT="$SCRIPT_DIR/../docs/check-agent-skills.sh"
TESTS_PASSED=0
TESTS_FAILED=0
TESTS_TOTAL=0
...
run_check() {
    local dir="${1:-$TEST_TMPDIR/.agents/skills}"
    local exit_code=0
...
test_clean_file_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/clean.md" << 'MDEOF'
# Clean Skill
...
test_unwrap_without_comment_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-unwrap.md" << 'MDEOF'
# Bad Unwrap
...
test_expect_without_comment_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-expect.md" << 'MDEOF'
# Bad Expect
...
test_unwrap_with_justifying_comment_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/ok-unwrap.md" << 'MDEOF'
# Justified Unwrap
...
test_unwrap_outside_rust_block_ignored() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/prose-unwrap.md" << 'MDEOF'
# Discussion
...
test_ambiguous_result_collect_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-collect.md" << 'MDEOF'
# Ambiguous Collect
...
test_explicit_result_collect_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/ok-collect.md" << 'MDEOF'
# Explicit Collect
...
test_catch_unwind_without_assert_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-unwind.md" << 'MDEOF'
# Bad Unwind
...
test_catch_unwind_with_assert_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/ok-unwind.md" << 'MDEOF'
# OK Unwind
...
test_line_count_over_limit_fails() {
    echo -e "${BOLD}Test: File over 500 lines is flagged${NC}"
    setup_tmpdir

    # Generate a file with 510 lines
    {
        echo "# Long File"
        for i in $(seq 1 510); do
            echo "Line $i of content"
        done
    } > "$TEST_TMPDIR/.agents/skills/long.md"

    assert_fail "file over 500 lines fails"
    assert_output_contains "violation mentions line-limit" "line-limit"
    teardown_tmpdir
}

test_line_count_at_limit_passes() {
    echo -e "${BOLD}Test: File at exactly 500 lines passes${NC}"
    setup_tmpdir

    # Generate a file with exactly 500 lines
    {
        for i in $(seq 1 500); do
            echo "Line $i"
        done
    } > "$TEST_TMPDIR/.agents/skills/exact.md"

    assert_pass "file at exactly 500 lines passes"
    teardown_tmpdir
}
...
test_dev_null_in_bash_block_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-devnull.md" << 'MDEOF'
# Bad Dev Null
...
test_dev_null_in_sh_block_fails() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/bad-devnull-sh.md" << 'MDEOF'
# Bad Dev Null Shell
...
test_dev_null_outside_shell_block_ignored() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/prose-devnull.md" << 'MDEOF'
# Discussion
...
test_fix_mode_shows_suggestions() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/fixable.md" << 'MDEOF'
# Fixable
...
test_multiple_violations_counted() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/multi.md" << 'MDEOF'
# Multiple Issues
...
test_empty_directory_passes() {
    ...
    # .agents/skills exists but no .md files
...
test_unwrap_with_justification_on_preceding_line_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/prev-line-just.md" << 'MDEOF'
...
test_unwrap_with_justification_several_lines_above_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/multi-line-just.md" << 'MDEOF'
...
test_unwrap_in_rust_comment_line_ignored() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/comment-unwrap.md" << 'MDEOF'
...
test_unwrap_with_allow_attribute_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/allow-attr.md" << 'MDEOF'
# Allow Attribute
...
test_unwrap_with_fixture_attribute_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/fixture-attr.md" << 'MDEOF'
# Fixture Attribute
...
test_unwrap_in_forbidden_block_passes() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/forbidden-demo.md" << 'MDEOF'
# Zero-Panic Policy
...
test_unwrap_still_flagged_without_justification() {
    ...
    cat > "$TEST_TMPDIR/.agents/skills/no-just.md" << 'MDEOF'
# No Justification
...
# ── Run all tests ─────────────────────────────────────────────────────────────
main() {
    echo -e "${BOLD}Running tests for check-agent-skills.sh${NC}"
    echo "================================================"
    echo ""
