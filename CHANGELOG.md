...
- The Tokio integration guide now makes the socket ownership boundary explicit: once a `TokioUdpSocket` moves into a session, the session uses its non-blocking trait methods and application code cannot call the standalone async helpers. The feature reference also documents `hot-join` and `trace-validation` and identifies the `loom` Cargo feature as a no-op compatibility name; actual Loom builds use `cfg(loom)`.
