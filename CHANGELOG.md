...
- Spectator failover via `SessionBuilder::start_spectator_session_multi(&[T::Address], socket)`: a spectator can connect to multiple redundant game peers ("hosts") at once. Confirmed inputs are accepted from whichever host delivers a frame first, and spectation continues while at least one host remains connected. A host that disconnects is removed (emitting a `FortressEvent::Disconnected` for it); when all hosts drop, already-buffered viewable frames may still drain before `advance_frame` returns `PredictionThreshold`. `SpectatorSession::num_hosts()` reports the current number of connected hosts so applications can observe redundancy in real time. The existing single-host `start_spectator_session` delegates to the same construction path and now also rejects invalid `SpectatorConfig` values. Robustness: newer frames may replace older ring-slot contents, but duplicate data for the same player/frame follows first-writer-wins semantics; identical duplicates are ignored, and divergent duplicates report a frame-sync violation without overwriting the first value. Shared connect-status / frame-advantage are refreshed only from the freshest host, while disconnected status still merges monotonically from lagging hosts so the earliest cutoff is preserved. If duplicate host addresses are supplied, inbound packets are routed to the first matching host endpoint.
- Enhanced spectator: `SpectatorConfig::stream_delay` holds playback back from the live edge by a configurable number of frames (anti-stream-sniping); the spectator never advances past `last_received_frame - stream_delay`, clamps the delayed boundary at `Frame::NULL` until enough host inputs arrive, and returns `PredictionThreshold` cleanly at the boundary. `SpectatorConfig::enable_rewind` makes the spectator emit a `SaveGameState` each frame and unlocks `SpectatorSession::seek_to_frame(Frame)`, which jumps to any previously simulated frame still in the buffer via a single `LoadGameState` (no re-simulation); seeking on a non-rewind spectator returns `InvalidRequestKind::NotSupported`, a negative target returns `InvalidFrameStructured`/`MustBeNonNegative`, `target + 1` overflow returns `FrameArithmeticOverflow`, and a frame outside the buffered window returns `InvalidFrameStructured`/`MissingState`. New accessors `SpectatorSession::is_rewind_enabled()` and `SpectatorSession::stream_delay()` expose the configured settings. `SpectatorConfig::validate()` rejects `buffer_size == 0`, frame-domain-overflowing `buffer_size` / `stream_delay` values, `stream_delay >= buffer_size`, and pathologically large `catchup_speed` / `max_frames_behind` values (each must be `<= 2_147_483_648`) that could otherwise drive an unbounded request-batch allocation in `advance_frame` (`catchup_speed == 0` remains valid); `start_spectator_session` and `start_spectator_session_multi` return `None` for invalid spectator configs. The catchup request-batch preallocation is additionally clamped to the buffer size at the call site, so even a directly constructed config that bypasses validation cannot trigger an oversized allocation. The spectator advance loop now also returns partial catchup progress instead of discarding already-gathered requests on a mid-batch unavailability; the degenerate `catchup_speed == 0` case (no frame is even attempted while behind) still returns `Ok(<empty>)` rather than `PredictionThreshold`. `seek_to_frame`'s seekable upper bound is `current_frame - 1`: seeking to the exact current frame returns `MissingState` because the post-state for that frame is not saved until the next advance.
- Hardened network input decompression against decompression-bomb denial of service: `rle::decode` (and therefore `network::compression::decode`, which is reached for every received input packet) now rejects malformed input whose declared decoded length exceeds an internal 64 MiB safety bound, returning an error instead of attempting an unbounded allocation that the default allocator would turn into a process abort. The bound is exposed as the new public constant `fortress_rollback::rle::MAX_DECODED_LEN`. Length accumulation also uses checked arithmetic so a crafted packet can no longer overflow `usize`.
...
- **Breaking:** `RleDecodeReason::DecodedLengthExceedsMaximum { decoded_len, max }` — new variant added; returned by `rle::decode` when malformed input declares a decoded length above the internal safety bound. Since `RleDecodeReason` is not `#[non_exhaustive]`, exhaustive matches must now handle this variant.
