...
All `wiki/*.md` files come from `docs/` via `scripts/sync-wiki.py`. Manual edits are overwritten.
...
```bash
python3 scripts/sync-wiki.py                 # Generate wiki
python3 scripts/check-wiki-consistency.py    # Cross-validate pipeline
python3 scripts/validate-wiki-output.py      # Check rendering issues
...
```bash
./scripts/check-links.sh                                    # All links
