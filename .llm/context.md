...
### Markdown Linting in Pre-Commit

**`markdownlint` runs automatically in pre-commit with auto-fix enabled.**

This means:

- Most formatting issues (extra blank lines, list spacing, etc.) are fixed automatically
- **MD051 (link-fragments)** errors for broken anchor links must be fixed manually
- If auto-fix changes files, you may need to `git add` the changes and re-commit
...
# Lint with auto-fix
npx markdownlint 'docs/user-guide.md' --config .markdownlint.json --fix
```
...
| MD051 | Link fragments must match headings     | Verify anchor matches actual heading     |

> **Note:** `markdownlint` runs in pre-commit with auto-fix enabled, automatically correcting many issues.

**Heading Anchor Generation Rules (MD051):**

When linking to headings with `#fragment` anchors, the anchor is generated by:

1. Convert to lowercase
2. Spaces become single hyphens (`-`)
3. Remove parentheses, brackets, and most special characters
4. **Slashes with spaces (`/`) become double hyphens (`--`)**
